...
# 加载数据
def load_data(sequence_file, ppi_file, sl_pairs_file, sl_non_pairs_file):
    ...
    return sequence_features, ppi_features, sl_pairs, sl_non_pairs

# 数据预处理
def preprocess_data(sequence_features, ppi_features, sl_pairs, sl_non_pairs, num_negative_samples=1):
    features = []
    labels = []
    pos_num = 0
    neg_num = 0
    ...
    for pair in sl_pairs:
        gene1 = pair[0]
        gene2 = pair[1]
        if gene1 in sequence_features and gene2 in sequence_features \
                and gene1 in ppi_features and gene2 in ppi_features:
            combined_features = np.concatenate([sequence_features[gene1], ppi_features[gene1], 
...
if __name__ == "__main__":
    ...
    sequence_features, ppi_features, sl_pairs, sl_non_pairs = load_data(sequence_file, ppi_file, sl_pairs_file, sl_non_pairs_file)
    print("成功加载数据！")
    features, labels = preprocess_data(sequence_features, ppi_features, sl_pairs, sl_non_pairs)
    # 检查features是否有无效值
    ...
    # 构建当前阈值的结果文件名
    results_filename = os.path.join(results_folder, f"results_1_0.004_{threshold}_mean_nonSTRUCT_1.txt")
