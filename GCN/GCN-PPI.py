...
# 加载数据
def load_data(structure_folder, sequence_file, sl_pairs_file, sl_non_pairs_file):
    structure_features = {}
    ...
    return structure_features, sequence_features, sl_pairs, sl_non_pairs

# 数据预处理
def preprocess_data(structure_features, sequence_features, sl_pairs, sl_non_pairs, num_negative_samples=1):
    features = []
    labels = []
    pos_num = 0
    neg_num = 0
    ...
    for pair in sl_pairs:
        ...
        if gene1 in structure_features and gene2 in structure_features \
            ...
            combined_features = np.concatenate([structure_features[gene1], sequence_features[gene1],
                                                structure_features[gene2], sequence_features[gene2]])
...
if __name__ == "__main__":
    ...
    structure_features, sequence_features, sl_pairs, sl_non_pairs = load_data(structure_folder, sequence_file, sl_pairs_file, sl_non_pairs_file)
    print("成功加载数据！")
    features, labels = preprocess_data(structure_features, sequence_features, sl_pairs, sl_non_pairs)
    # 检查features是否有无效值
    ...
    # 构建当前阈值的结果文件名
    results_filename = os.path.join(results_folder, f"results_1_0.004_{threshold}_mean_nonPPI.txt")
